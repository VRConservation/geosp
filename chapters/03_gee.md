...
You need to do a quick exploratory data analysis of above and below-ground carbon biomass for an area of interest near Lake Tahoe in the Central Sierra, California. A donor has asked you to do this by the end of the week. You go down the hall to the right where the map plotters are and ask the GIS guy. He peers up above his array of screens and says he's busy working on a dozen other projects; he might be able to get you something in 2 weeks. He suggests you check out Google Earth Engine.

Yes! You've heard of this and think Google Earth will be easy. But when you check out Earth Engine and find a console with an empty map, the required Javascript coding is another universe, let alone language.
...
Clicking run will give you a basic map of aboveground biomass:
...
In this case, the Mosaic image has 4 bands related to above and belowground carbon, the latitude and longitude of a point clicked is (-68.16, 1.83), and the pixel size is 10km. Add in the belowground (bgb) carbon with
...
Hit run or control + enter (windows) or command + enter (mac) to run the new code. Cleaning up the code and adding comments with // to explain what each section does and standard coding practice (the double back slash allows you to add a comment that does not run when the code executes and is equivalent to the hashtag for Python). You can comment out any line of code by clicking on the line and typing control/command enter on your keyboard. A reorganization will give you the following code and image:
...
- **Awesome-GEE**. The Opengeos [Awesome-GEE](https://github.com/opengeos/Awesome-GEE) Github repo is an incredible curated list of GEE resources. This incredible resource has everything from getting started to courses, papers, datasets, and more. You could lose yourself for days in here!
— **GEE Google Group**. The Google Earth Engine Developers [listserv](https://groups.google.com/g/google-earth-engine-developers) is quite useful for answering many other questions.
...
- **Visualization and Analysis of Brazil Floods 2024**. There is an excellent workflow design along with superb explanations of remote sensing theory and concepts in this [tutorial](https://share.gishub.org/brazil_floods/#create-sentinel-1-sar-composites). In addition, the tutorial contains very useful text and code to analyzing synthetic aperature radar (SAR) imagery.
-- **RiverREM Python Package**. Available [here](https://github.com/OpenTopography/RiverREM). Autpomates riverrelative elevation model (REM) visualizations from DEM model input.
