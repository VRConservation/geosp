...
## Similarities
Javascript, Python, and R have strikingly similar code structures ({numref}`comparison`).
...
## lidR
Lidar (Light detection and ranging) is frequently used for forest stand analysis. Let's explore the lidR package to see its amazing 3D visualization and analysis capabilities. Not only does the package come with a book with excellent instructions to demo the package functions, it also has a manageable sized las point cloud dataset {cite}`lidr`. The following examples are excerpted from the book.
