...
It's similar enough to organize each coding language in your head, almost like learning Spanish and Portuguese. It's probably better to focus on one or the other. Still, you may find that one language sometimes offers better packages, processes data faster than the other, or is preferred in a particular workplace.


```{tip} R has the somewhat annoying symbol for variables or assignment operator of <-. Still, you can quickly enter it using the keyboard shortcut Alt/Option and - (Alt and minus sign) in Windows/Mac.
```
...
Download the [chocoloate bar ratings](https://www.kaggle.com/datasets/rtatman/chocolate-bar-ratings/data) dataset from kaggle and copy the path to wherever you stored it.
...
- **[Data wrangling recipes in R](http://gg.gg/1b5fyf)**. A nice resource for data preparation in R by Hilary Watt.
